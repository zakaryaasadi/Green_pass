...
public class SquareProgressView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isIndeterminate()) {
            Path path = new Path();
            DrawStop drawEnd = getDrawEnd((scope / 200) * Float.valueOf(String.valueOf(indeterminate_count)), canvas);


            if (drawEnd.place == Place.TOP) {
                path.moveTo(cW + strokewidth - drawEnd.location, cH - hSw);
            ...
            if (drawEnd.place == Place.RIGHT) {
                path.moveTo(hSw, cH + strokewidth - drawEnd.location);
                path.lineTo(hSw, hSw);
            ...
            if (drawEnd.place == Place.BOTTOM) {
                path.moveTo(cW + strokewidth - drawEnd.location,
                        hSw);
                path.lineTo(cW - hSw, hSw);
                path.lineTo(cW - hSw, cH - hSw);
                path.lineTo( drawEnd.location - strokewidth, cH
                        - hSw);
                canvas.drawPath(path, progressBarPaint);
            }

            if (drawEnd.place == Place.LEFT) {
                path.moveTo(cW - hSw, cW + strokewidth - drawEnd.location);
                path.lineTo(cW - hSw, cH - hSw);
                path.lineTo(hSw, cH - hSw);
                path.lineTo(hSw, drawEnd.location -hSw);
                canvas.drawPath(path, progressBarPaint);
            }

            indeterminate_count++;
            if (indeterminate_count > 200) {
